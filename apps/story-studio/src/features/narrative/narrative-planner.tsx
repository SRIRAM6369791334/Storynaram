...
import { useNarratives, useCreateNarrative, useDeleteNarrative } from '@/hooks/use-narratives';
...
export function NarrativePlanner() {
  ...
  const deleteNarrative = useDeleteNarrative();
  ...
  const handleCreate = async () => {
    ...
    try {
      const narr = await createNarrative.mutateAsync({ title, description });
      openTab({ id: `narrative-${narr.id}`, title: narr.title, type: 'narrative', entityId: narr.id });
      setShowCreate(false);
      setTitle('');
      setDescription('');
  ...
  return (
    <div className="space-y-2">
      ...
      {!narratives || narratives.length === 0 ? (
        <p className="text-xs text-muted-foreground py-4 text-center">No narratives yet. Plan your story arcs.</p>
      ) : (
        narratives.map((narr) => (
          <div
            key={narr.id}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium">{narr.title}</p>
                {narr.description && <p className="text-xs text-muted-foreground">{narr.description}</p>}
              </div>
            </div>
            <Button size="icon" variant="ghost" className="opacity-0 group-hover:opacity-100" onClick={(e) => { e.stopPropagation(); deleteNarrative.mutate(narr.id); }}>
